...
Endstop endstopY(Y_MIN_PIN, Y_DIR_PIN, Y_STEP_PIN, Y_ENABLE_PIN, Y_MIN_INPUT, Y_HOME_STEPS, HOME_DWELL, false);
  ...
  //Endstop endstopZ(Z_MIN_PIN, Z_DIR_PIN, Z_STEP_PIN, Z_ENABLE_PIN, true, Z_HOME_STEPS, HOME_DWELL, false);
#endif

//EQUIPMENT OBJECTS
#if GRIPPER == SERVO
  Servo_Gripper gripper(SERVO_PIN, SERVO_GRIP_DEGREE, SERVO_UNGRIP_DEGREE);
#elif GRIPPER == BYJ
  BYJ_Gripper gripper(BYJ_PIN_0, BYJ_PIN_1, BYJ_PIN_2, BYJ_PIN_3, BYJ_GRIP_STEPS);
#elif GRIPPER == RAMPS
  Ramps_Gripper gripper(E0_STEP_PIN, E0_DIR_PIN, E0_ENABLE_PIN, INVERSE_E0_STEPPER, 1, 1, 1, 2400);
#endif
...
void executeCommand(Cmd cmd) {
  ...
  if (cmd.id == 'G') {
    ...
    case 3: gripper.cmdOn(); break;
    case 5: gripper.cmdOff(); break;
...
void homeSequence_UNO(){
  #if GRIPPER == SERVO
  if (gripper.readDegree() != SERVO_UNGRIP_DEGREE){
    gripper.cmdOff();
  }
  #endif
  if (HOME_Y_STEPPER && HOME_X_STEPPER){
    while (!endstopY.state() || !endstopX.state()){
      Logger::logINFO("HOMING YX...");
...
void ps4_controller_loop(){
  ...
  if (controller_ps4.buttons[PS4_CROSS]){if (GRIPPER == SERVO){gripper.cmdOn();}}
  if (controller_ps4.buttons[PS4_CIRCLE]){if (GRIPPER == SERVO){gripper.cmdOff();}}
